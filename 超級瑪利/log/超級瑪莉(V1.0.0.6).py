...
# 版本(寫好玩的)
version = Version.render("V1.0.0.6", True, (0, 0, 0))     # 放在這裡純粹方便改
...
jump_delay = 8 * (clock_hz / 60)                       # 長按大跳時長判定 (5~20就好)
...
class physics :
    ...
    def __init__(self, object_map, trap_map, player_sizex, player_sizey, player_x, player_y, player_image_1, player_image_2) :
        # player 基礎設置
        self.player_sizex = player_sizex
        self.player_sizey = player_sizey
        self.player_image_1 = player_image_1
        self.player_image_2 = player_image_2
        # player 判定
        ...
        # 創建玩家精靈
        self.player = Player(self.player_x, self.player_y, '.\Cat-Mario\超級瑪利\cat_right.png')
        # # 加載圖片(.convert_alpha() << 背景透明化用的)
        # self.image_player = pygame.image.load('.\Cat-Mario\超級瑪利\cat.png').convert_alpha()
        # # 獲取圖像矩形對象
        # self.rect_player = self.image_player.get_rect(topleft=(self.player_x, self.player_y))
        # # 建立像素掩碼
        # self.player_mask = pygame.mask.from_surface(self.image_player)

    def keypress_model(self) :
        keys = pygame.key.get_pressed()
        if keys[pygame.K_LEFT] :
            self.key_left = 1
            player_1.player.change_image( '.\Cat-Mario\超級瑪利\cat_left.png')
        else :
            self.key_left = 0
        if keys[pygame.K_RIGHT] :
            self.key_right = 1
            player_1.player.change_image( '.\Cat-Mario\超級瑪利\cat_right.png')
        else :
            self.key_right = 0
        if keys[pygame.K_UP] :
            self.key_up = 1
    ...
    def keymove_model (self) : 
        ...
        if self.hold == 1 and self.jump == 1 and jump == 1:
            self.velocity_y = 0
...
# 建立
game_loopset =  physics(0, 0, 0, 0, 0, 0, 0, 0)
# 地圖檔
    # map_1
map_1_object = [
        [0,  20, 300000, 20 ],
...
key_1 = physics(0, 0, 0, 0, 0, 0, 0, 0)      
#(object_map=map_1_object, player_sizex=10, player_sizey=10, player_x=1, player_y=(height - 110))
player_image_1 = '.\Cat-Mario\超級瑪利\cat_right.png'
player_image_2 = '.\Cat-Mario\超級瑪利\cat_left.png'
player_1 = physics(map_1_object, map_1_trap, 30, 45, 1, 410, player_image_1, player_image_2) 

# 創建玩家精靈
# player = Player(player_1.player_x, player_1.player_y, '.\Cat-Mario\超級瑪利\cat.png')
all_sprites.add(player_1.player)
